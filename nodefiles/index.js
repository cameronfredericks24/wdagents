...
const { parsePrompt } = require("./Aiparser");
const { deployToSalesforce } = require("./deploytosalesforce");
const { writeFileRecursive } = require("./utils");
const {
  generateCustomObjectXML,
  generateCustomFieldXML,
  generateApprovalProcessXML
} = require("./metadataGenerators");
...
function validateApprovalProcess(details) {
  ...
  for (const [i, step] of details.steps.entries()) {
    ...
    for (const approver of step.approvers) {
      ...
      if (type === "relatedUserField" && !approver.name) 
        return { valid: false, error: `Step ${i + 1} relatedUserField missing field name.` };
...
app.post("/api/generate-metadata", async (req, res) => {
  ...
  try {
    ...
    if (action === "approval") {
      console.log('[DEBUG] Approval Process Generation: objectName:', details.objectName, 'processName:', details.processName, 'steps:', JSON.stringify(details.steps));
      ...
      // Log the first 20 lines of the generated XML
      console.log('[DEBUG] Generated Approval Process XML (first 20 lines):', xml.split('\n').slice(0, 20).join('\n'));
      ...
      if (!fs.existsSync(path.join(PROJECT_ROOT, deployPath))) {
    ...
    if (action === "object") {
      ...
      const objectFilePath = path.join(
        objectDir,
        `${details.objectName}.object-meta.xml`
      );
      ...
      for (const field of details.fields || []) {
        ...
        const fieldPath = path.join(
          fieldsDir,
          `${field.fieldName}.field-meta.xml`
        );
        writeFileRecursive(fieldPath, fieldXml);
      }

      const deployObjectPath = `force-app/main/default/objects/${details.objectName}/`;
      const stdout = await deployToSalesforce(deployObjectPath);
      // Only return minimal, user-friendly details
      const simpleFields = (details.fields || []).map(f => {
        const out = {
          fieldName: f.fieldName,
          label: f.label,
          type: f.type
        };
        if (f.type && f.type.toLowerCase() === 'picklist' && f.values) out.values = f.values;
        if (f.type && f.type.toLowerCase() === 'lookup' && f.referenceTo) out.referenceTo = f.referenceTo;
        return out;
      });
      return res.json({
